...
class AlexNet(object):
    ...
    def __init__(self, x, keep_prob, num_classes, skip_layer, do_bn, is_train,
    ...
    def create(self):
        ...
        if (self.DO_BN == True):
            print "Yes batch norm - is_train:", self.IS_TRAIN
            #apply batch norm
            bn6 = self.batch_norm(dropout6, 'bn6', self.IS_TRAIN)

            # 7th Layer: FC (w ReLu) -> Dropout
            fc7 = fc(bn6, 4096, 4096, name='fc7')
            dropout7 = dropout(fc7, self.KEEP_PROB)

            #apply batch norm
            bn7 = self.batch_norm(dropout7, 'bn7', self.IS_TRAIN)

            # 8th Layer: FC and return unscaled activations
            self.fc8 = fc(bn7, 4096, self.NUM_CLASSES, relu=False, name='fc8')
        else:
            print "No batch norm"
            # 7th Layer: FC (w ReLu) -> Dropout
